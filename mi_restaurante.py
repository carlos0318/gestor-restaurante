...
def total():
    ...
    subtotal =  sub_total_postres + sub_total_comida + sub_total_bebidas
...
def guardar():
    ...
    archivo.close()
    messagebox.showinfo('Informacon', 'Su recibo ha sido guardado')
...
lista_postres = ['helado', 'fruta', 'brownies', 'flan', 'mousse', 'pastel1', 'pastel2', 'pastrel3']
...
texto_costo_comida.grid(row=0, column=1, padx=1)
...
texto_costo_bebida.grid(row=1, column=1, padx=1)
...
texto_costo_postre.grid(row=2, column=1, padx=1)
...
texto_subtotal.grid(row=0, column=3, padx=1)
...
texto_impuesto.grid(row=1, column=3, padx=1)
...
texto_total.grid(row=2, column=3, padx=1)
